...
make_cooccurrence_plot <- function(df, title=NULL, original_label=1, predicted_label.x=1, predicted_label.y=1,
  ...
  g <- cooccurrence %>%
    ...
    labs(x="Model x country", y="Model y country", title=title, 
...
10,000 donors
```{r}
g <- make_cooccurrence_plot(df_with_pred2, title="Number of true deferrals that both models detect in Finnish data")
g
...
g <- make_cooccurrence_plot(full_data_df_with_pred2, 
                            title="Number of true deferrals both models detect in Finnish data")
g
...
```{r}
g <- make_cooccurrence_plot(df_with_pred2, title="Number of true deferrals model x detects in Finnish data but model  y does not",
...
```{r}
g <- make_cooccurrence_plot(full_data_df_with_pred2, title="Number of true deferrals model x detects in Finnish data but model y does not",
                            original_label=1, predicted_label.x=1, predicted_label.y=0)
g
