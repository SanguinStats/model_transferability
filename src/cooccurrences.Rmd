...
g <- make_cooccurrence_plot(full_data_df_with_pred %>% filter(Model %in% models),
                            ...
                            title="Number of attempts \nlabeled the same way \nin Dutch data")
g
if (save_figs) ggsave(sprintf("%s/same-labels-predicted-full.png", result_path), g, width=width, height=height, unit="mm")
```
...
g <- make_cooccurrence_plot(full_data_df_with_pred %>% filter(Model %in% models), 
                            ...
                            original_label=1, predicted_label.x=1, predicted_label.y=0)
g# + coord_fixed()
if (save_figs) ggsave(sprintf("%s/x-better-full-data.pdf", result_path), g, width=width, height=height, unit="mm")
```
