...
aupradjplot <- ggplot(dfadj, aes(x=value, 
  ...
  labs(title="Performance  by dataset of models trained in different countries",color = "Model trained in") +
...
aupradjplot <- annotate_figure(aupradjplot, right = grobTree(
  ...
  textGrob("Model evaluated in", rot = -90, x = 0.7, vjust = 1, gp = gpar(col = "black"))
))
...
p <- annotate_figure(p, right = text_grob("Model evaluated in", rot = -90, size = 10, x = -9.75, vjust=1))
