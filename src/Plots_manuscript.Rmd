...
data <- bind_rows(datalist) %>%
  ...
  select(-defrate_train, -defrate_test, -defrate_validate, -AUPR.value, -AUPR.high, -AUPR.low)%>%
  rename("value"="AUPRadj.value","low"="AUPRadj.low","high"="AUPRadj.high") %>% mutate(metric="Adjusted AUPR")

data2 <- bind_rows(datalist) %>%
  select(modelcountry, datacountry, Sex, AUROC.value, AUROC.low, AUROC.high, model) %>%
  rename("sex"="Sex")%>%
  mutate(modelcountry = case_when(modelcountry=='BE' ~ 'Belgium',
         ...
         datacountry = case_when(datacountry=='BE' ~ 'Belgium',
                             ...
                             datacountry=='AU2' ~ 'Australia (EU cut off)',
                             datacountry=='SA2'~ 'South Africa (Own cut off)')) %>%
  rename("value"="AUROC.value","low"="AUROC.low","high"="AUROC.high") %>% mutate(metric="AUROC")
...
datafull <- datafull %>% mutate(modelcountry = fct_relevel(modelcountry, "Australia (EU cut off)","Australia (Own cut off)","Belgium","Finland","Netherlands","South Africa (EU cut off)", "South Africa (Own cut off)"))
...
aupradjplot <- ggplot(datafull, aes(x=value, 
             xmin=low, 
             xmax=high, 
             y = modelcountry,
             color = modelcountry, shape=metric)) +
  geom_pointrange(position = position_dodge(width= 0.0), size = 1, fatten = 1.0) +
  labs(title="Performance  by dataset of models trained in different countries",
       shape = "Performance metric") +
  coord_cartesian(xlim=c(0, 1)) + 
  facet_grid(datacountry ~ sex, labeller = labeller(datacountry = datacountry.labs, sex = sex.labs)) + xlab("Performance") + ylab("Model trained in") + guides(color="none", shape = guide_legend(override.aes = list(size = 0.7)))+scale_y_discrete(limits = rev(levels(datafull$modelcountry)))

aupradjplot <- annotate_figure(aupradjplot, right = text_grob("Model evaluated in", rot = -90, size = 10, vjust = 1, x = -7.8))
...
auprplot <- annotate_figure(auprplot, right = text_grob("Model evaluated in", rot = -90, size = 10, vjust = 1, x = -7.8))
...
df <- datafull %>% select(sex,datacountry,modelcountry) %>% filter(metric == "AUROC") %>%
  mutate(modelcountry = fct_relevel(modelcountry, "Australia (EU cut off)","Australia (Own cut off)","Belgium","Finland","Netherlands","South Africa (EU cut off)", "South Africa (Own cut off)"))
...
aurocplot<-  ggplot(df, aes(x=value, 
  ...
  labs(title="Performance  by dataset of models trained in different countries",
       color = "Model trained in") +
  coord_cartesian(xlim=c(0, 1)) + 
  facet_grid(datacountry ~ sex, labeller = labeller(datacountry = datacountry.labs, sex = sex.labs)) + xlab("AUROC") + ylab("Model trained in") + theme(legend.position = "none") + scale_y_discrete(limits = rev(levels(df$modelcountry)))
aurocplot <- annotate_figure(aurocplot, right = text_grob("Model evaluated in", rot = -90, size = 10, vjust = 1, x = -7.8))
...
p <- annotate_figure(p, right = text_grob("Model evaluated in", rot = -90, size = 10, vjust = 1, x = -7.8))
...
```{r SHAP plots}
  plt <- ggplot(data=data_maa, aes(x = reorder(variable, avg_maa))) +facet_grid(datacountry ~ sex, labeller = labeller(datacountry = datacountry.labs, sex = sex.labs))+
      ...
      coord_flip() + xlab("Variable") + ylab("Mean absolute attribution")+ theme_bw()+ ggtitle("Variable importance in different models on datasets from various countries")+
      labs(color = "Model from", shape = "Model from") + theme(axis.text.x=element_text(size=8))
plt<- annotate_figure(plt, right = text_grob("Model evaluated in", rot = -90, size = 10, vjust = 1, x = -7.8))
